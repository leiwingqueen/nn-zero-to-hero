...
画成网络图是这样的 —— 输入层 27 个神经元、输出层 27 个 logits、中间是一个 27×27 的全连接权重 `W`，一共 729 个可训练参数（没有隐藏层，没有 bias）：

![神经网络前向传播](nn_forward.png)

*以输入字符 `e`（ix=5）为例：one-hot 只有第 5 位是 1，所以只有它发出的 27 条连线真正起作用 —— 其余 26 个输入的贡献都被乘成了 0。右侧节点大小和条形长度是训练后的真实概率，模型认为 `e` 后面最可能接 `.`（19.5%，即名字到此结束）、`l`（15.9%）、`n`（13.1%）。*
