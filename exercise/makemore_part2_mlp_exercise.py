...
def main():
    ...
    # --- 1. 数据集构造 ---
    Xe, Ye = build_dataset(["emma"], stoi)
    assert Xe.tolist() == [[0, 0, 0], [0, 0, 5], [0, 5, 13], [5, 13, 13], [13, 13, 1]], \
        f"'emma' 的 X 不对：{Xe.tolist()}"
    assert Ye.tolist() == [5, 13, 13, 1, 0], f"'emma' 的 Y 不对：{Ye.tolist()}"
    assert Xe.dtype == torch.int64 and Ye.dtype == torch.int64, "X/Y 应为整数张量（下标）"
    ...
    assert Xtr.shape[1] == BLOCK_SIZE, "X 的第二维应等于 block_size"
    total = Xtr.shape[0] + Xdev.shape[0] + Xte.shape[0]
    # 228146 = 所有 bigram 样本数，part1 里也是这个数（每个名字贡献 len(w)+1 条）
    assert total == 228146, f"三个 split 的样本总数期望 228146，实际 {total}"
    assert Xtr.shape[0] / total > 0.79, "训练集应占约 80%"
    print("[1/6] 数据集构造 OK")

    # --- 2. 参数初始化 ---
    params = init_params()
    C, W1, b1, W2, b2 = params
    assert C.shape == (VOCAB_SIZE, N_EMB), f"C 形状应为 (27, {N_EMB})"
    assert W1.shape == (BLOCK_SIZE * N_EMB, N_HIDDEN), "W1 形状不对"
    assert b1.shape == (N_HIDDEN,) and b2.shape == (VOCAB_SIZE,), "偏置形状不对"
    assert W2.shape == (N_HIDDEN, VOCAB_SIZE), "W2 形状不对"
    assert all(p.requires_grad for p in params), "所有参数都要 requires_grad=True"
    nparams = sum(p.nelement() for p in params)
    print(f"      参数量: {nparams}")
    assert nparams == 11897, f"参数量期望 11897，实际 {nparams}"
    print("[2/6] 参数初始化 OK")

    # --- 3. 前向传播 ---
    logits = forward(Xtr[:32], params)
    assert logits.shape == (32, VOCAB_SIZE), f"logits 形状应为 (32, 27)，实际 {tuple(logits.shape)}"
    logits_all = forward(Xdev[:1000], params)
    assert logits_all.shape == (1000, VOCAB_SIZE), "forward 必须能处理任意 batch 大小"
    init_loss = F.cross_entropy(forward(Xtr[:1000], params), Ytr[:1000]).item()
    print(f"      初始 loss = {init_loss:.4f}（随机初始化下 loss 很大，part3 会专门讲怎么修）")
    assert init_loss > 3.0, "随机初始化时 loss 应该明显大于 -log(1/27)=3.30 附近"
    print("[3/6] 前向传播 OK")

    # --- 4. 学习率搜索 ---
    lrs, losses = find_lr(Xtr, Ytr, steps=1000)
    assert len(lrs) == 1000 and len(losses) == 1000, "find_lr 应返回两个等长的 list"
    best_lr = float(lrs[losses.index(min(losses))])
    print(f"      扫描到的最优 lr ≈ {best_lr:.4f}（课程结论：0.1 是个不错的起点）")
    assert 0.01 < best_lr <= 1.0, f"最优 lr 落在了奇怪的位置：{best_lr}"
    print("[4/6] 学习率搜索 OK")

    # --- 5. 训练 ---
    print("      开始训练 10 万步（CPU 约 25 秒）...")
    lossi = train(Xtr, Ytr, params, steps=100000, lr=0.1)
    assert len(lossi) == 100000, "train 应返回每一步的 loss"
